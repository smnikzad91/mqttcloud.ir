...
export default function AdminTicketDetailPage() {
  ...
  if (!ticket) {
    return (
      <div className="space-y-4">
        <Link href="/admin/tickets" className="text-sm text-brand-500 hover:underline">
          ← {t("backToTickets")}
        </Link>
  ...
  return (
    <div className="space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      {/* Back */}
      <Link href="/admin/tickets" className="inline-flex items-center gap-1 text-sm text-brand-500 hover:underline">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {ticket.status !== "closed" ? (
              ...
              <button
                ...
                disabled={updating}
                className="rounded-lg border border-gray-200 px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors disabled:opacity-50"
              >
                {t("reopenTicket")}
              </button>
            )}
        ...
        {/* User info */}
        <div className="mt-4 flex items-center gap-3 rounded-xl bg-gray-50 px-4 py-3 dark:bg-gray-800">
      ...
      <div className="ticket-scroll space-y-3 max-h-[520px] overflow-y-scroll pr-3">
        ...
        <div className="flex justify-end">
          <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-gray-100 dark:bg-gray-800 px-4 py-3">
            <p className="mb-1 text-xs font-semibold text-gray-500">{userName}</p>
        ...
        {ticket.replies.map((r) => (
          <div
            key={r._id}
            className={`flex ${r.sender === "user" ? "justify-end" : "justify-start"}`}
          >
            <div
              className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                r.sender === "user"
                  ? "rounded-tr-sm bg-gray-100 dark:bg-gray-800"
                  : "rounded-tl-sm bg-brand-500"
              }`}
            >
              {r.sender === "user" && (
                <p className="mb-1 text-xs font-semibold text-gray-500">{userName}</p>
              )}
              <p className={`text-sm whitespace-pre-wrap ${r.sender === "user" ? "text-gray-800 dark:text-white" : "text-white"}`}>
                {r.message}
              </p>
              ...
              <p className={`mt-1 text-xs ${r.sender === "user" ? "text-gray-400" : "text-white/60"}`}>
