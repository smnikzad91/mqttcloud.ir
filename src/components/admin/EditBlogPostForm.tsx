...
export default function EditBlogPostForm({ id }: Props) {
  const t = useT();
  ...
  const [form, setForm] = useState({
    ...
    readTime: "", published: true, highlight: false,
  });
  ...
  useEffect(() => {
    fetch(`/api/admin/blog/${id}`)
      ...
      .then((data) => {
        ...
        const loaded = (data.sections ?? []).map((s: { heading?: string; body: string; image?: string }) => ({
          heading: s.heading ?? "",
          body: s.body,
          image: s.image ?? "",
        }));
        setSections(loaded);
        setSectionUploading(loaded.map(() => false));
        setLoading(false);
      });
  }, [id]);
  ...
  if (loading) return <div className="p-6 py-20 text-center text-gray-400">{t("blogLoading")}</div>;

  return (
    <div className="p-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("blogEditPostTitle")}</h1>
          <p className="mt-1 font-mono text-xs text-gray-400">{form.slug}</p>
        </div>
        {form.highlight && (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900 space-y-4">
          ...
          <div>
            ...
            <textarea required rows={3} value={form.excerpt} onChange={(e) => set("excerpt", e.target.value)} className={inputClass} placeholder={t("blogFieldExcerptPlaceholder")} />
          </div>
          <div>
            ...
            {sections.map((section, i) => (
              <div key={i} className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  {sections.length > 1 && (
                    <button type="button" onClick={() => removeSection(i)} className="text-xs text-red-400 hover:text-red-500">{t("blogSectionRemove")}</button>
                  )}
                </div>
                <input value={section.heading} onChange={(e) => setSection(i, "heading", e.target.value)} className={`${inputClass} mb-2`} placeholder={t("blogSectionHeadingPlaceholder")} />
                <textarea required rows={4} value={section.body} onChange={(e) => setSection(i, "body", e.target.value)} className={`${inputClass} mb-3`} placeholder={t("blogSectionBodyPlaceholder")} />
                <div>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button type="submit" disabled={saving} className="rounded-xl bg-brand-500 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-600 disabled:opacity-50">
            {saving ? t("blogSavingPost") : t("blogSaveChanges")}
          </button>
          <button type="button" onClick={() => router.push("/admin/blog")} className="rounded-xl border border-gray-200 px-6 py-2.5 text-sm font-semibold text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800">
            {t("blogCancelPost")}
          </button>
        </div>
      </form>
