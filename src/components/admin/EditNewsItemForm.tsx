...
export default function EditNewsItemForm({ id }: Props) {
  const t = useT();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/admin/news/${id}`, {
      ...
      body: JSON.stringify({ ...form, hashtags, image: image || null, coverImage: coverImage || null }),
    });
  ...
  if (loading) return <div className="p-6 py-20 text-center text-gray-400">{t("newsLoading")}</div>;

  return (
    <div className="p-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("newsEditItemTitle")}</h1>
        {form.highlight && (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-3 pt-2">
          <button type="submit" disabled={saving} className="rounded-xl bg-brand-500 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-600 disabled:opacity-50">
            {saving ? t("newsSavingItem") : t("newsSaveChanges")}
          </button>
